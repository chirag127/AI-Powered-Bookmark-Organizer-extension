    ...
    try {
        ...
        const categorizedBookmarks = await Promise.all(
            newBookmarks.map((bookmark) =>
                geminiService.categorizeBookmark(bookmark)
            )
        );
...
const updateBookmark = async (req, res, next) => {
    try {
        ...
        if (updateData.title || updateData.url) {
            bookmarks[bookmarkIndex] = await geminiService.categorizeBookmark(
                bookmarks[bookmarkIndex]
            );
        }
...
const archiveBookmarksBatch = (req, res, next) => {
    try {
        ...
        const bookmarksToArchive = bookmarks.filter((b) =>
...
module.exports = {
    ...
    restoreBookmark,
    deleteBookmark,
    // Export the bookmarks arrays for direct access by other controllers
    bookmarks,
    archivedBookmarks,
};
