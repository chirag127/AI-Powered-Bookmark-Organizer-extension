...
async function categorizeBookmarks(bookmarks) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || 'Failed to categorize bookmarks');
    }
...
async function checkBookmarksForArchiving() {
  try {
    ...
    const thresholdDate = Date.now() - (settings.autoArchiveDays * 24 * 60 * 60 * 1000);
...
async function archiveBookmarks(bookmarkIds) {
  try {
    const response = await fetch(`${API_BASE_URL}/bookmarks/archive-batch`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
